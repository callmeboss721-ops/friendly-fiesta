...
assert(assertMoney({ minorUnits: 0, currency: 'USD' }).minorUnits === 0, 'zero minor units are valid');
assertThrows(() => assertMoney({ minorUnits: 0, currency: 'USDT' }), 'non-ISO settlement asset is rejected by Money');
...
const reconciliationEvent = transitionReconciliation({ entityId: 'd1', from: 'MATCHED', to: 'READY', actorId: 'reviewer', occurredAt: new Date('2026-09-12T00:00:00.000Z') });
assert(reconciliationEvent.occurredAt === '2026-09-12T00:00:00.000Z', 'audit event is deterministic');
